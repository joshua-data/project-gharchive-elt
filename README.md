...
flowchart LR
    ...
    GHA -->|"dbt build (manual dispatch, paused)"| BQ
...
7. **dbt** (`dbt-run.yml`, manual `workflow_dispatch` only — the daily `17 6 * * *` UTC schedule was removed in September 2026) impersonates `gharchive-dbt-runner` via WIF, runs `dbt build --target prod --vars '{batch_date: <yesterday UTC>}'` against BigQuery, materializing curated facts and dimensions into `dw`. Docs regeneration/publishing was tied to the removed push trigger, so the hosted dbt docs stay frozen at the last automated publish.
