...
- **`cloud_run_job_memory = "2Gi"`, `cloud_run_job_timeout = "1800s"`, `catchup_hours = 12`.** Sized after the 2026-09 OOM incident, when GH Archive started inlining issue/PR bodies and an hourly archive went from 36 MB to 400 MB uncompressed. The container was OOM-killed at the old `1Gi` limit, and because each run processes its window oldest-first, every subsequent run died on the same hour — a 32-hour stall that only surfaced when `dbt source freshness` failed the daily build. The ingest path is now streaming end to end (peak RSS ~345 MB regardless of payload size — see [`ingest/README.md`](../ingest/README.md)), so `2Gi` is headroom rather than a ceiling that tracks upstream growth. Remember Cloud Run mounts `/tmp` as tmpfs, so spooled files count against it. `catchup_hours = 12` widens the gap the pipeline heals on its own; the timeout has to cover 13 hours of work in the worst case, hence `1800s`.
